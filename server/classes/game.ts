  ...
  findMoves() {
    this.board.forEach((row, x) => {
      row.forEach((square, y) => {
        square.legalMoves = findPossibleMoves(
          this.board,
          this.history[this.history.length > 1 ? this.history.length - 2 : 0],
          {
            x,
            y,
          }
    ...
    this.board.forEach((row, x) => {
      row.forEach((square, y) => {
        ...
        square.legalMoves = findPossibleMoves(
          ...
          if (
            newLegalMoves.some((row, i) =>
              row.some((box, j) =>
                box.some((newLegalMove) => {
                  return (
                    (newBoard[newLegalMove.x][newLegalMove.y].piece ===
                      "king" &&
